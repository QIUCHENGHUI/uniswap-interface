...
const DisclaimerModal: React.FC<DisclaimerModal> = ({ onConfirm, onDismiss = () => {} }) => {
    ...
    if (step === 'disclaimer') {
      return (
        <div>
          <p>Audits: None.</p>
          <p>
            While the initial creators of the Yam protocol have made reasonable efforts to attempt to ensure the
            security of the contracts, including forking much of the codebase from existing well-audited projects and
            soliciting review from friends, nothing approaching the rigor of a formal audit has been conducted at this
            time.
          </p>
  ...
  const button = useMemo(() => {
    if (step === 'disclaimer') {
      return <Button text="Next" variant="secondary" onClick={() => setStep('uniswap')} />
    } else {
