...
export const approve = async (tokenContract, poolContract, account) => {
  return tokenContract.methods
    .approve(poolContract.options.address, ethers.constants.MaxUint256.toHexString())
...
export const getCirculatingSupply = async yam => {
  const now = await yam.web3.eth.getBlock('latest')
  const scalingFactor = yam.toBigN(await yam.contracts.yam.methods.yamsScalingFactor().call())
  const starttime = yam.toBigN(await yam.contracts.eth_pool.methods.starttime().call()).toNumber()
  let timePassed = now['timestamp'] - starttime
  if (timePassed < 0) {
    return 0
  }
  const yamsDistributed = yam.toBigN((8 * timePassed * 250000) / 625000) //yams from first 8 pools
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const starttimePool2 = yam.toBigN(await yam.contracts.ycrv_pool.methods.starttime().call()).toNumber()
  timePassed = now['timestamp'] - starttime
  const pool2Yams = yam.toBigN((timePassed * 1500000) / 625000) // yams from second pool. note: just accounts for first week
  const circulating = pool2Yams
...
export const getNextRebaseTimestamp = async yam => {
  try {
    const now = await yam.web3.eth.getBlock('latest').then(res => res.timestamp)
    const interval = 43200 // 12 hours
    const offset = 28800 // 8am/8pm utc
    ...
    if (await yam.contracts.rebaser.methods.rebasingActive().call()) {
      ...
      const twap_init = yam.toBigN(await yam.contracts.rebaser.methods.timeOfTWAPInit().call()).toNumber()
      if (twap_init > 0) {
        const delay = yam.toBigN(await yam.contracts.rebaser.methods.rebaseDelay().call()).toNumber()
        const endTime = twap_init + delay
