      ...
      <CardContent>
        <StyledResponsiveWrapper>
          ...
          <StyledCenter>
            ...
            >
              <div style={{ fontSize: 12 }}>{`${Number(
                totalVotes.multipliedBy(scalingFactor).toFixed(0)
              ).toLocaleString()}`}</div>
              <div
                style={{
                  fontSize: 12,
        ...
        {!delegated ? (
          ...
          <div>
            <StyledDelegatedCount>
              Delegating: {Number(delegatedBalance.multipliedBy(scalingFactor).toFixed(0)).toLocaleString()} YAM
            </StyledDelegatedCount>
        ...
        >
          <StyledLink target="__blank" href="https://twitter.com/YamFinance/status/1293660938906869760">
            More Info
          </StyledLink>
        </div>
      </CardContent>
...
const StyledCheckpoint = styled.div<StyledCheckpointProps>`
  ...
  left: ${({ left }) => left}%;
  z-index: 1;
  top: ${({ reverse }) => (reverse ? 20 : 0)}px;
  color: ${({ theme, acheived }) => (acheived ? theme.secondaryMain : theme.grey600)};
  &:after {
    content: '';
    position: absolute;
    width: 1px;
    background-color: ${({ theme, acheived }) => (acheived ? theme.secondaryMain : theme.grey400)};
    height: 28px;
    left: 0;
    top: ${({ reverse }) => (reverse ? -32 : 40)}px;
  }
`
